...
/*
1) Clauses:
Predicates: vertical, line, horizontal, point
Rules:
Facts:

2) Constants: None
Variables: X, Y, Z
Complex Structs: point(X,Z) line(X, Y) line(point(X,Y),point(X,Z)) etc
*/
...
is_increasing([]).
is_increasing([_]).
is_increasing([X, Y | Z]) :- increased(X, Y), !, is_increasing([Y | Z]).
...
zip2([], [_|_], []).
zip2([_|_], [], []).
zip2([Hx | Tx], [Hy | Ty], [Hz | Tz]) :- ((Hx, Hy) = Hz), !, zip2(Tx, Ty, Tz).
